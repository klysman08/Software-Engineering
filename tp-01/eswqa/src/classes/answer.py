...
class Answer:
    ...
    def _insert(self, idquestion, iduser, description):
        return self.db.sql(
            f'INSERT INTO Answer(idquestion, iduser, description) VALUES ("{idquestion}", "{str(iduser)}", "{description}")'
        )

    def _delete(self, answer_id, user_id):
        return self.db.sql(
            f'DELETE FROM Answer WHERE idanswer = {str(answer_id)} AND iduser = {str(user_id)}'
        )

    def _edit(self, answer_description, answer_id):
        return self.db.sql(
            f'UPDATE Answer SET description="{answer_description}" WHERE idanswer = "{str(answer_id)}"'
        )

    def get_by_user(self, user_id):
        return self.db.query(
            f'SELECT a.idquestion, a.idanswer, a.description, DATE_FORMAT(a.data, "%d/%m/%Y %H:%i:%s") AS data, q.title, u.fullname FROM Answer a INNER JOIN Question q ON a.idquestion = q.idquestion INNER JOIN User u ON a.iduser = u.iduser WHERE a.iduser = "{user_id}"'
        )

    def remove_by_question_id(self, question_id):
        return self.db.sql(f'DELETE FROM Answer WHERE idquestion = {str(question_id)}')

    def get_iduser_by_idanswer(self, answer_id):
        return self.db.sql(
            f'SELECT iduser FROM Answer WHERE idanswer = "{str(answer_id)}"'
        )

    def get_by_id(self, answer_id):
        return self.db.query(
            f'SELECT q.title, a.iduser, a.description FROM Answer a INNER JOIN Question q ON a.idquestion = q.idquestion WHERE a.idanswer = "{str(answer_id)}"'
        )[0]
    ...
    def validate_answer_edit(self, description, user_id, answer_id):
        if not self.utils.validate_not_empty([description, user_id, answer_id]):
            return False
        return self._edit(description, answer_id) if user_id else False
    ...
    def validate_answer_post(self, idquestion, iduser, description):
        if not self.utils.validate_not_empty([idquestion, iduser, description]):
            return False
        return self._insert(idquestion, iduser, description) if iduser else False
