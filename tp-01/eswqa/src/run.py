...
def pergunta(pergunta_id):
    ...
    if request.method == 'POST':
        ...
        return popup(
            ...
            links=[{'url': f'/pergunta/{str(pergunta_id)}', 'text': 'Voltar'}],
        )
    else:
        answer = Answer.Answer()
...
def fazer_pergunta():
    ...
    dict_tag = {'Tags': {f'tag {str(i)}': item for i, item in enumerate(tags)}}
    dict_tag = json.dumps(dict_tag)


    if not question.validate_question_post(
        request.form['title'],
        request.form['body'],
...
def editar_pergunta(pergunta_id):
    ...
    if not question.validate_question_post(
        ...
        return popup(
            msg="Erro ao editar pergunta",
            links=[
                {
                    'url': f'/editar-pergunta/{str(pergunta_id)}',
...
def editar_resposta(resposta_id):
    ...
    if not answer.validate_answer_edit(
        request.form['description'],
        session.get('logged_user_id'),
        str(resposta_id),
    ):
        return popup(
            msg="Erro ao editar resposta",
            links=[
                {
                    'url': f'/editar-resposta/{str(resposta_id)}',
                    'text': 'Tentar Novamente',
                }
            ],
        )
...
def minha_conta():
    if request.method == 'POST':
        ...
        if session.get('logged_user_id'):
            user = User.User()
            if data := user.get_by_id(str(session.get('logged_user_id'))):
...
def remover_pergunta(pergunta_id):
    return popup(
        msg="Tem certeza que quer remover a sua pergunta?",
        links=[
            {
                'url': f'/remover-pergunta-confirmado/{str(pergunta_id)}',
                'text': 'Remover pergunta!!!',
            },
            {'url': '/minhas-perguntas', 'text': 'Voltar'},
        ],
    )
...
def remover_resposta(resposta_id):
    return popup(
        msg="Tem certeza que quer remover a sua resposta?",
        links=[
            {
                'url': f'/remover-resposta-confirmado/{str(resposta_id)}',
                'text': 'Remover resposta!!!',
            },
            {'url': '/minhas-respostas', 'text': 'Voltar'},
        ],
    )
