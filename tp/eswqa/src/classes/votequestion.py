...
class VoteQuestion:
    ...
    def _insert(self, idquestion, iduser, vote):
        return self.db.sql(
            f'INSERT INTO VoteQuestion(idquestion, iduser, vote) VALUES ({idquestion},{iduser},{vote})'
        )

    def _select_vote(self, idquestion, iduser):
        return int(
            self.db.query(
                f'SELECT COUNT(*) AS COUNT FROM VoteQuestion WHERE idquestion = {idquestion} AND iduser = {iduser}'
            )[0]['COUNT']
        )
