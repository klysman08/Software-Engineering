...
class Question:
    ...
    def _edit(self, question_title, question_description, question_id, question_tag):
        print(f'{question_tag}hihi')
        print(
            f"UPDATE Question SET title='{question_title}', description='{question_description}', tags='{question_tag}' WHERE idquestion = {str(question_id)}"
        )
        return self.db.sql(
            f"UPDATE Question SET title='{question_title}', description='{question_description}', tags='{question_tag}' WHERE idquestion = {str(question_id)}"
        )
